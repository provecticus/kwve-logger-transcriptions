...
REM Prefer console health; fallback to API if console port not defined
...
call :log "OS=%OS_URL%  OSD=%OSD_URL%  MINIO_READY=%MINIO_READY%  PG %POSTGRES_DB%@%POSTGRES_PORT%"
...
REM Prefer console health; fallback to S3 API root HEAD on 19000 if console health not 200
call :wait_http "MinIO Ready" "%MINIO_READY%" "200" 20 2
if errorlevel 1 (
  echo.
  echo [INFO] MinIO /ready (console) not 200; trying /live ...
  call :wait_http "MinIO Live" "%MINIO_LIVE%" "200" 40 2
  if errorlevel 1 (
    echo.
    echo [INFO] Falling back to S3 HEAD / on API port...
    call :wait_http "MinIO S3 HEAD /" "http://localhost:%MINIO_PORT%/" "200,204,301,302" 40 2
  )
)

REM ----- (3) OpenSearch index & doc -----
echo [3] Checking OpenSearch index/doc...
call :curl_code "%OS_URL%/kwve-transcripts"
...
call :curl_code "%OS_URL%/kwve-transcripts/_doc/radio:KWVE:2025-09-04T09:00:00Z"
